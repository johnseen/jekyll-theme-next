...
1. 安装jenkins
```console
    helm install stable/jenkins -f ~/kubetest/helm/jenkins-values.yaml
```
2. 修改jenkins-values配置文件
...
## 配置jenkins
1. 安装kubernetes Continuous Deploy,插件名为kubernetes-cd.hpi
   ![手动安装插件](http://ww1.sinaimg.cn/large/005WXpR7ly1gahznn5xapj31g80nwmy8.jpg)
2. 挂载本机/var/run/docker.sock,修改Slave空闲回收时间,防止工作节点自动回收
   ![示例](http://ww1.sinaimg.cn/large/005WXpR7ly1gahzgjc51lj312m0ne0u2.jpg) 
3. 配置jenkins访问kubernetes秘钥配置文件
   ![jenkins-k8s](http://ww1.sinaimg.cn/large/005WXpR7ly1gahzk1bpipj316i0k8acv.jpg)
4. 配置jenkins访问gitlab秘钥
   ![jenkins-gitlab](http://ww1.sinaimg.cn/large/005WXpR7ly1gahzkvlvhhj30jl0b474c.jpg)


## 创建任务
1. 创建pipeline任务
   ![创建任务](http://ww1.sinaimg.cn/large/005WXpR7ly1gahzqor046j30r40h9ta2.jpg)
2. 参数化构建过程
    ![配置构建过程](http://ww1.sinaimg.cn/large/005WXpR7ly1gahzskkqbwj314u0crjrq.jpg)
3. 编写pipeline脚本
   ```yaml
   		pipeline {
		  agent any
		  stages {
		    stage('Checkout Source') {
		      steps {
		        git credentialsId: '2001', url: 'http://192.168.100.155/luxsun/jenkinstest.git'  
		    }
		
		    stage('Deploy App') {
		      steps {
		        script {
		          kubernetesDeploy(configs: "scm-appointment-all.yaml", kubeconfigId: "1001") 
		        }
		  ...
		  environment {
		    registry = 'registry.cn-hangzhou.aliyuncs.com/scm-oms/luxsun'
		    dockerImage = ''
		  }
		}
   ``` 
